...
import { useTheme } from "../context/ThemeContext"; // 🎯 THE FIX: Import our global theme context hook

export default function Navbar() {
const { user, logout } = useUserAuth(); // 🎯 THE FIX: Dropped legacy dark/setDark from Auth
  const { isDarkMode, toggleTheme } = useTheme(); // 🎯 THE FIX: Extract true theme controls
 ...
 const handleLogout = async () => {
    try {
      // 1. Instantly collapse dropdown menus
      setActiveDropdown(null);
      setMobileMenuOpen(false);

      // 2. Clear the global Auth context state (setUser(null) and call backend API)
      await logout();

      // 3. Force navigate to the login screen immediately after context resolves
    ...
    >
      <div className="h-full px-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* 🎯 THE FIX: Rewired button actions to toggleTheme and read isDarkMode */}
          <button
            onClick={toggleTheme}
            className="
              w-11
          ...
          {user ? (
            ...
            >
              <button className="flex items-center gap-3">
                ...
                <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-600 text-white font-bold flex items-center justify-center shadow-lg">
                  {getUserInitial()}
                </div>
              </button>
