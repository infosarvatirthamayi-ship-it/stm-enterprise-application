...
export default function AdminLogin() {
  ...
  // 🎯 Wired to Super Admin Context (Stateless Theme Included)
  const { admin, login, dark, setDark } = useAdminAuth();
  ...
  return (
    <div className={`min-h-screen relative flex items-center justify-center px-4 overflow-hidden transition-colors duration-500 ${dark ? 'dark bg-slate-950' : 'bg-slate-100'}`}>
      ...
      <div className={`relative z-10 w-full max-w-[420px] rounded-[2.5rem] border backdrop-blur-2xl shadow-2xl p-10 transition-all duration-500 ${dark ? 'bg-white/5 border-white/10 shadow-black/50' : 'bg-white/70 border-white/50 shadow-indigo-500/10'}`}>
        <div className={`absolute -top-20 -left-20 w-48 h-48 rounded-full blur-[80px] transition-all duration-500 ${dark ? 'bg-indigo-600 opacity-50' : 'bg-indigo-400 opacity-30'}`} />
        ...
        <div className="text-center relative z-10 mt-2">
          ...
          <h1 className={`text-3xl font-black tracking-tight font-serif transition-colors ${dark ? 'text-white' : 'text-slate-800'}`}>System Core</h1>
          <p className={`text-[10px] mt-2 mb-8 uppercase tracking-[0.2em] font-black transition-colors ${dark ? 'text-slate-400' : 'text-slate-500'}`}>Global Admin Command</p>
        </div>
